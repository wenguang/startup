...
- (id<WXImgLoaderProtocol>)imageLoader
{
    ...
    dispatch_once(&onceToken, ^{
        imageLoader = [[ImageHandler alloc] init];
    });
    return imageLoader;
}
                      ...
                      withContainer:(UIViewController *)container
{
    ...
    WeexViewController *vc = [[WeexViewController alloc] initWithSourceURL:[NSURL URLWithString:param[@"url"]]];
...
- (void)setNavigationBarHidden:(BOOL)hidden animated:(BOOL)animated
                 withContainer:(UIViewController *)container
{
    //If weex launchs in the ARKWeexViewController, the navigationBar is hidden according to our design.
    if (![container isKindOfClass:[WeexViewController class]]) {
        return;
    }
        ...
        withContainer:(UIViewController *)container
{
    if (param[@"title"]) {
        ...
        BarButton *button = [BarButton buttonWithType:UIButtonTypeRoundedRect];
...
{
    BarButton *button = (BarButton *)sender;
